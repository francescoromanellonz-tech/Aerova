...
colors:
  ...
  cream: "#EDF3F8"
  white-tinted: "#FAFAF8"
  text-main-light: "#1F2F3D"
  text-sub-light: "#3B5268"
  text-main-dark: "#E8E6E1"
  text-sub-dark: "#A0ADB5"
typography:
  display:
...
- Dual-register color: light mode is Restrained, dark mode is Committed.
...
- AA contrast in both modes (sage and water-crystal are darkened in light mode for AA).
- Honors `prefers-reduced-motion` and `Save-Data` at the JS layer.

## 2. Colors: The Atmospheric Palette

A two-mode palette built on tinted neutrals, anchored by a single warm metal
(champagne gold) and a single cool elemental (water crystal). The light mode
breathes; the dark mode broadcasts.
...
- **Champagne Gold** (`#D4AF37`): the brand's only warm metal. Used on
  ...
  and section break flourishes. Its rarity is the point: ≤10% of any light-
  mode surface, slightly higher in dark mode where it carries cinematic
  weight.

### Secondary

- **Morning Sage** (`#5E7A6E` light mode, `#8DA399` dark mode): the brand's
  ambient hue. Carries surface tints, button outlines, badges, and large-
  area washes. Darkened in light mode to reach AA contrast on text.

### Tertiary

- **Water Crystal** (`#5A96A8` light mode, `#7AB8C8` dark mode): the
  elemental cool. Carries the Vietnamese tagline color, water-quality badges,
  scroll progress bar, focus outlines, and the dot-glow on How-It-Works.
- **Water Deep** (`#3D7A8E`): badge text on water-crystal surfaces.
  Rarely a fill.

### Neutral

- **Obsidian Matte** (`#1A1A1B`): dark mode background; primary text in
  certain inverted contexts; the visual gravity of the brand.
- **Slate Charcoal** (`#334155`): legacy primary text token; today
  `text-main-light` (`#1F2F3D`) carries body text in light mode for AA.
- **Cream** (`#EDF3F8`): light mode background. Cooler than warm cream,
  closer to fresh-water pale.
- **White-Tinted** (`#FAFAF8`): card surfaces and inverted text on dark.
  Never pure `#FFFFFF`.
...
the cinematic dark mode; pure white shatters the cream warmth of light mode.

**The Dual-Register Rule.** Light mode runs Restrained — cream and tinted
sage neutrals, gold accent ≤10%. Dark mode runs Committed — obsidian
dominates 60%+ of every surface, gold expands to carry cinematic weight.
Designs are dual-tested at this strategy boundary.
...
- **Gold (`aerova-btn--gold`):** Same shape, champagne-gold border, filled
  gold background in dark mode (transparent in light, with gold border).
  Used as the primary CTA on the hero and main quote/order entry points.
...
- **Purity Float Badge (`purity-float`):** Sharp rectangle, frosted
  white background (`rgba(255,255,255,0.68)`) with `backdrop-filter:
  blur(18px) saturate(180%)`, 1px gold border, gold uppercase text at 9px
...
- **Background:** White-tinted in light mode (`#FAFAF8` or
  `rgba(255,255,255,0.96)`), sage-tinted in dark mode
  (`rgba(141,163,153,0.05)`).
- **Border:** Faint gold borders on premium surfaces
  (`rgba(212,175,55,0.28)` light, `0.22` dark). Sage borders on
...
- **Style:** Liquid-glass effect (`backdrop-filter: blur(16px) saturate(180%)`)
  with theme-aware background tint. Shrinks on scroll. Includes a
...
- **Canvas Frame Hero:** A 145-frame scroll-driven canvas animation that
  ...
  `Save-Data` gating, and RAF cleanup. The cinematic centerpiece in dark
  mode. On mobile, opacity reduced and a stronger left-gradient overlay is
  applied to keep the text area legible.
...
- **Do** use sharp 0-radius rectangles for buttons and cards. The hard
  edge is the brand.
- **Do** apply dual-register color: light mode Restrained (gold ≤10%),
  dark mode Committed (obsidian dominates).
...
- **Do** target WCAG 2.2 AA contrast in both light and dark modes — use
  the darkened sage and water-crystal tokens (`#5E7A6E`, `#5A96A8`) in
  light mode, the vibrant tokens (`#8DA399`, `#7AB8C8`) in dark mode.
