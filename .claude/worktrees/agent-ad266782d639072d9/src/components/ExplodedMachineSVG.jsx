...
/*
 * Chassis geometry — slim vertical slab, centered in a SQUARE viewBox.
 * The viewBox is 1280×1280 (1:1) so modules have horizontal breathing
 * room around the slim chassis.
 *
 ...
 */
const CHASSIS = {
  x: 570, y: 90, w: 140, h: 1100,    // centered: (1280 - 140) / 2 = 570
  topRadius: 4,
  cx: 640,                           // centerline x
  ...
  alcoveTop:      460,
  alcoveBottom:   580,
  alcoveCenterY:  500,               // chrome lever vertical center
  trimBandY:      610,
  frontDoorTop:   620,
 ...
 */
/*
 * Anchor positions point to the TRUE anatomical location of each module
 * on the chassis (per manual page 7). Targets are spread around the wider
 * 1280×1280 canvas for visual clarity. Modules alternate left/right of
 * the chassis to avoid line crossings.
 *
 * Chassis spans x=570..710 horizontally, so:
 *   - "left" targets sit around x=180..360
 *   - "right" targets sit around x=920..1100
 */
const MODULE_LAYOUTS = [
  {                   // 01 Air intake — anchors to LEFT-SIDE air-inlet mesh
    num: '01',
    label: 'Air intake',
    anchor: { x: 570, y: 340 },     // left edge of chassis at mesh midpoint
    target: { x: 280, y: 240 },
    align:  'left',
    accent: WATER_CRYSTAL,
  },
  {                   // 02 Cooling coils — anchors to TOP of chassis (coils inside top)
    num: '02',
    label: 'Condensation',
    anchor: { x: 640, y: 90  },     // chassis top center
    target: { x: 1000, y: 200 },
    align:  'right',
    accent: WATER_CRYSTAL,
  },
  {                   // 03 Sediment + Pre-Carbon — anchors to UPPER front door
    num: '03',
    label: 'Pre-filtration',
    anchor: { x: 710, y: 720 },     // chassis right edge at upper front door
    target: { x: 1000, y: 540 },
    align:  'right',
    accent: GOLD,
  },
  {                   // 04 Ultra-fine membrane — anchors to MID front door
    num: '04',
    label: 'Ultra-fine membrane',
    anchor: { x: 570, y: 880 },     // chassis left edge at mid front door
    target: { x: 280, y: 880 },
    align:  'left',
    accent: WATER_CRYSTAL,
  },
  {                   // 05 UV-C + minerals — anchors to LOWER front door
    num: '05',
    label: 'UV-C and minerals',
    anchor: { x: 710, y: 1060 },    // chassis right edge at lower front door
    target: { x: 1000, y: 1080 },
    align:  'right',
    accent: GOLD,
  },
  {                   // 06 Hot+cold dispense — anchors RIGHT ON the chrome levers
    num: '06',
    label: 'Hot + cold dispense',
    anchor: { x: 640, y: 500 },     // dead center of dispense alcove (chrome lever zone)
    target: { x: 280, y: 580 },
    align:  'left',
    accent: SAGE,
  },
...
const ExplodedMachineSVG = forwardRef(function ExplodedMachineSVG(_, ref) {
  return (
    ...
    >
      ...
      <g opacity="0.4">
        <text x="60" y="44" fill={TEXT_SUB} fontSize="11" letterSpacing="2.5" fontFamily="Nunito, sans-serif" style={{ textTransform: 'uppercase' }}>
          LT-AWG20G · EXPLODED VIEW
        </text>
        <text x="1220" y="44" textAnchor="end" fill={TEXT_SUB} fontSize="11" letterSpacing="2.5" fontFamily="Nunito, sans-serif" style={{ textTransform: 'uppercase' }}>
          A · 2026
        </text>
      </g>
      <g opacity="0.4">
        <line x1="60" y1="1240" x2="200" y2="1240" stroke={TEXT_SUB} strokeWidth="0.7"/>
        <text x="60" y="1258" fill={TEXT_SUB} fontSize="10" letterSpacing="1.8" fontFamily="Nunito, sans-serif" style={{ textTransform: 'uppercase' }}>
          1138mm overall height · 375mm wide · 307mm deep
        </text>
      </g>
    </svg>
  );
